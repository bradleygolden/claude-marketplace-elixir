defmodule Mix.Tasks.Claude.Hooks.RunTest do
  ...
  describe "webhook reporter integration" do
    setup do
      Mimic.copy(Claude.Hooks.Reporter)
      :ok
    end

    test "calls Reporter.dispatch when reporters are configured", _context do
      config = %{
        hooks: %{
          post_tool_use: ["test.success"]
        },
      ...
      stub(Claude.Hooks.Reporter, :dispatch, fn event_data, config ->
        send(test_pid, {:reporter_dispatch_called, event_data, config})
        :ok
      end)
      ...
      assert_received {:reporter_dispatch_called, ^event_data, ^config}
    end

    test "calls Reporter.dispatch for different hook events", _context do
      config = %{
        hooks: %{
          stop: ["test.check"]
        },
    ...
    test "does not call Reporter.dispatch when no reporters configured", _context do
      config = %{
        hooks: %{
          post_tool_use: ["test.success"]
        }
    ...
    test "calls Reporter.dispatch even when hooks fail", _context do
      config = %{
        hooks: %{
          pre_tool_use: ["test.failure"]
        },
        reporters: [
          {:webhook, url: "https://example.com/webhook"}
        ]
    ...
    test "passes event data with correct structure to Reporter.dispatch", _context do
      config = %{
        hooks: %{
          post_tool_use: ["format"]
        },
      ...
      stub(Claude.Hooks.Reporter, :dispatch, fn received_event_data, received_config ->
        send(test_pid, {:reporter_dispatch_called, received_event_data, received_config})
        :ok
      end)
      ...
      assert_received {:reporter_dispatch_called, received_event_data, received_config}

      assert received_event_data["hook_event_name"] == "post_tool_use"
      assert received_event_data["tool_name"] == "Edit"
      assert received_event_data["session_id"] == "abc123"
      assert received_event_data["tool_input"]["file_path"] == "lib/test.ex"

      assert received_config[:reporters] == [{:webhook, url: "https://example.com/webhook"}]
    end
  end
end
