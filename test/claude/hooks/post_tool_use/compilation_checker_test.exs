defmodule Claude.Hooks.PostToolUse.CompilationCheckerTest do
  ...
  describe "run/1" do
    ...
    test "works with .exs files", %{test_dir: test_dir} do
      file_path = create_elixir_file(test_dir, "test.exs", """
      IO.puts("Hello, World!")
      """)
    ...
    test "works with MultiEdit tool", %{test_dir: test_dir} do
      file_path = create_elixir_file(test_dir, "lib/multi.ex", """
      defmodule Multi do
        def test, do: :ok
      end
      """)
    ...
    test "ignores non-edit tools", %{test_dir: test_dir} do
      file_path = create_elixir_file(test_dir, "lib/read.ex", """
      defmodule Read do
        def test, do: :ok
      end
      """)
    ...
    test "uses CLAUDE_PROJECT_DIR when available", %{test_dir: test_dir} do
      ...
      file_path = create_elixir_file(test_dir, "lib/subdir/test.ex", """
