...
# Test: Pre-commit check skips when Ash not in dependencies
test_hook \
  "Pre-commit check: Skips when Ash not in dependencies" \
  "plugins/ash/scripts/pre-commit-check.sh" \
  "{\"tool_input\":{\"command\":\"git commit -m 'test'\"},\"cwd\":\"$REPO_ROOT/test/fixtures/no-deps-project\"}" \
  0 \
  ""

# Test: Post-edit check skips when Ash not in dependencies
test_hook_json \
  "Post-edit check: Skips when Ash not in dependencies" \
  "plugins/ash/scripts/post-edit-check.sh" \
